...
// with the weights of members is published and each month he is the last on the list which means he is the 
// heaviest.

// I am the one who establishes the list so I told him: "Don't worry any more, I will modify the order of the 
// list". It was decided to attribute a "weight" to numbers. The weight of a number will be from now on the sum 
// of its digits.
...
const orderWeight = (str) => {
    ...
    for(let totalNumber of strArr){
        let sum = 0
        for(let number of totalNumber){
            sum += Number(number)
        }
        arr.push([sum, totalNumber])
    }
...
const orderWeight = (str) => {
    ...
    .map((v) => {
        return [
            v.split("").reduce((acc, curr) => acc + Number(curr),0),
            v
        ]
    })
    .sort((a,b) => {
